...
@df.region()
def top():
    fifo_A = df.array(df.pipe(dtype=T, shape=(), depth=4), shape=(P0, P1))
    fifo_B = df.array(df.pipe(dtype=T, shape=(), depth=4), shape=(P0, P1))

    @df.kernel(mapping=[P0, P1])
    def gemm(A: T[M, K], B: T[K, N], C: T[M, N]):
        i, j = df.get_pid()
        # periperals kernels
...
def test_systolic():
    ...
    if hls.is_available("vitis_hls"):
        ...
        #mod = s.build(target="vitis_hls", mode="sw_emu", project="systolic.prj")
        mod = s.build(target="vitis_hls", mode="csim", project="systolic_lstest_fixed.prj")
        # C = np.zeros((M, N), dtype=np.float32)
        # mod(A, B, C)
        # np.testing.assert_allclose(C, np.dot(A, B), atol=1e-5)
        print("Build Passed!")
